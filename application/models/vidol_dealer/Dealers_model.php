...
if (! defined('BASEPATH'))
	...
	{
		private $table_name = 'Dealers_tbl';
		private $fields_pk = 'd_pk';
		...
		public function get_row_by_title_status ($select, $title)
		{
			if(!empty($select)){
				$this->r_db->select($select);
			}
			$this->r_db->where('d_title', $title);
			$this->r_db->where('d_status', '1');
			$query = $this->r_db->get($this->table_name);
			//echo $this->r_db->last_query();
			if ($query->num_rows() > 0){
				return $query->row();
