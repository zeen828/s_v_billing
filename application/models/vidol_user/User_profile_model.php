...
class User_profile_model extends CI_Model {
	private $table_name = 'User_profile_tbl';
	private $fields_pk = 'u_pk';
	...
	public function update_for_data($pk, $data) {
	...
	public function get_row_by_mongoid_memberid($select, $mongo_id, $member_id) {
		if (! empty ( $select )) {
			$this->r_db->select ( $select );
		}
		if (empty ( $member_id )) {
			$this->r_db->where ( 'u_mongo_id', $mongo_id );
			//$this->r_db->where ( 'u_status', '1' );
		} else {
			$this->r_db->where ( 'u_member_id', $member_id );
			//$this->r_db->where ( 'u_status', '1' );
		}
		$query = $this->r_db->get ( $this->table_name );
		// echo $this->r_db->last_query();
